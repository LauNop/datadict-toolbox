...
def getDatasource(database_elmt, namespace):
    # Récupérer la datasource
    str_datasource = database_elmt.find("{}DataSources".format(namespace)).find("{}DataSource".format(namespace)).find(
        "{}ConnectionString".format(namespace)).text
...
def cubeDimensionsUsage(cube, catalog, cube_struct, namespace, src_database, src_serv, database_elmt):
    ...
    for c_dim in c_dims:
        ...
        for attrib in attribs:
            ...
            for i in range(len(dim_dict[dimension_id]["Attribute_id"])):
                print(dim_dict[dimension_id]["Attribute_id"][i], ":", group_name, ":", column_name, ":",
                      dim_dict[dimension_id]["Attribute_id"][i] == column_name)

                if dim_dict[dimension_id]["Attribute_id"][i] == column_name:
                    data_type = dim_dict[dimension_id]["Datatype"][i]
                    src_column = dim_dict[dimension_id]["Column"][i]
                    src_table = dim_dict[dimension_id]["Table"][i]
                    new_values = [column_name, "", data_type, "wip", is_measure, is_dimension, expression, "wip",
                                  group_name, cube_name, catalog, f"{src_column}/{src_table}/{src_database}/{src_serv}"]
...
def cubeMeasures(cube, catalog, cube_struct, namespace, src_database, src_serv):
    ...
    for measure_group in measure_groups:
        ...
        for measure in measures:
            ...
            new_values = [column_name, "", data_type, "wip", is_measure, is_dimension, expression, "wip", group_name,
                          cube_name, catalog, f"{src_column}/{src_table}/{src_database}/{src_serv}"]
...
def extractCubeMultidimStructure(file_path):
    # Structure de dictionnaire en sortie
    cube_struct = {"ATTRIBUTE_NAME": [], "NOM_EXPLICIT": [], "DATA_TYPE": [], "IS_CALCULATED": [], "IS_MEASURE": [],
                   "IS_DIMENSION": [], "EXPRESSION": [], "IS_VISIBLE": [], "GROUP": [], "CUBE_NAME": [],
                   "CATALOG_NAME": [], "SOURCE": []}
